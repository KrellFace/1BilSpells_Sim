...
class ModNode(SpellNode):
    def __init__(self, nodeName: str, max_parent_nodes: int, max_child_nodes: int, power_mod: float, elemental_type: enum_ElementalType = None, elemental_mod: float = None):
    ...
    def transmit_energy(self, energy_packet: EnergyPacket):
        ...
        if len(self._child_nodes)>0:
            out_packet = EnergyPacket((energy_packet.getPower()*self._power_mod)/len(self._child_nodes), energy_packet.getElementalTendency())
...
class DamageNode(SpellNode):
    ...
    def transmit_energy(self, energy_packet: EnergyPacket):
        #print("Transmitting energy in node " + self._nodeName)
        packet_list = []
        self_packet = self.generate_info_packet(energy_packet)

        #ONLY TRIGGER DAMAGE AND CHILDREN IF INPUT POWER IS HIGH ENOUGH
        ...
        if(self_packet.directDamage>MIN_DAMAGE_THRESHOLD):
            ...
            for child in self._child_nodes:
                packet_list.extend(child.transmit_energy(energy_packet))
        #print("Dealing " + str(energy) + " damage to  " + str(self._aoe_radius) + " max target")
        return packet_list
    ...
    def generate_info_packet(self, energy_packet: EnergyPacket): 
        ...
        return SpellInfoPacket(get_damage_from_energypacket(energy_packet, self._power_mod), get_max_targets_for_aoe_radius(self._aoe_radius))
    ...
    def copy_node(self):
        ...
        return DamageNode(self._nodeName, self._max_child_nodes, self._aoe_radius, self._power_mod)
...
class StaticDamageNode(SpellNode):
    ...
    def transmit_energy(self, energy_packet: EnergyPacket):
        ...
        total_ticks = math.floor(energy_packet.getPower()*self._ticks_per_power*energy_packet.getSpecificElementalTendency(enum_ElementalType.WATER))
        #print("Total ticks == " + str(total_ticks) +" energy packet power: " + str(energy_packet.getPower()) + " tpp: " + str(self._ticks_per_power))


        for x in range(total_ticks):
            packet_list.append(self.generate_info_packet(energy_packet))
            for child in self._child_nodes:
                packet_list.extend(child.transmit_energy(EnergyPacket(energy_packet.getPower()*self._power_mod, energy_packet.getElementalTendency())))
        #print("Dealing " + str(energy) + " damage to  " + str(self._aoe_radius) + " max target")
        return packet_list
    ...
    def copy_node(self):
        return StaticDamageNode(self._nodeName, self._max_child_nodes, self._aoe_radius, self._ticks_per_power, self._power_mod)
